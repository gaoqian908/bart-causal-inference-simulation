# R code for 5 methods in 3 response surfaces 
...
results.a <- matrix(0,niters,16)
results.b <- matrix(0,niters,16)
results.c <- matrix(0,niters,16)
dimnames(results.a) <- list(NULL,c("b.te","b.cov","b.cil","r.te","r.cov","r.cil","aipw.te","aipw.cov","aipw.cil","ipw.te","ipw.cov","ipw.cil","tau.est","aipw+b.te","aipw+b.cov","aipw+b.cil"))
dimnames(results.b) <- list(NULL,c("b.te","b.cov","b.cil","r.te","r.cov","r.cil","aipw.te","aipw.cov","aipw.cil","ipw.te","ipw.cov","ipw.cil","tau.est","aipw+b.te","aipw+b.cov","aipw+b.cil"))
dimnames(results.c) <- list(NULL,c("b.te","b.cov","b.cil","r.te","r.cov","r.cil","aipw.te","aipw.cov","aipw.cil","ipw.te","ipw.cov","ipw.cil","tau.est","aipw+b.te","aipw+b.cov","aipw+b.cil"))


precision.a <- matrix(0,niters,5)
dimnames(precision.a) <- list(NULL,c("bart","reg","aipw","ipw","aipw+b"))
precision.b <- matrix(0,niters,5)
dimnames(precision.b) <- list(NULL,c("bart","reg","aipw","ipw","aipw+b"))
precision.c <- matrix(0,niters,5)
dimnames(precision.c) <- list(NULL,c("bart","reg","aipw","ipw","aipw+b"))
...
for(i in 1:niters){
  ...
  #AIPW+BART
  # Create test sets: everyone as treated and everyone as control 
  xt1 <- cbind(X_good, Z_good = rep(1, n_good))
  xt0 <- cbind(X_good, Z_good = rep(0, n_good))
  
  # Train BART model
  bart2a <- dbarts::bart(x.train = xt, y.train = YA, x.test = rbind(xt1, xt0), verbose = FALSE)
  
  # Extract predicted E[Y|X, Z=1] and E[Y|X, Z=0] for all units
  mu1_hat <- colMeans(bart2a$yhat.test[, 1:n_good])
  mu0_hat <- colMeans(bart2a$yhat.test[, (n_good + 1):(2 * n_good)])
  # Compute AIPW estimate
  aipw_te <- mean(
    mu1_hat - mu0_hat +
      Z_good * (YA - mu1_hat) / ps_est -
      (1 - Z_good) * (YA - mu0_hat) / (1 - ps_est))
  # Store AIPW results
  results.a[i, 14] <- tauAs - aipw_te
  aipw_var <- var(mu1_hat - mu0_hat + Z_good * (YA - mu1_hat) / 
                    ps_est - (1 - Z_good) * (YA - mu0_hat) / (1 - ps_est)) / n_good
  ci <- c(aipw_te - 1.96 * sqrt(aipw_var), aipw_te + 1.96 * sqrt(aipw_var))
  results.a[i, 15] <- (ci[1] < tauAs & ci[2] > tauAs) * 1
  results.a[i, 16] <- ci[2] - ci[1]
  precision.a[i, 5] <- sqrt(mean((aipw_te - tauAis)^2))
  
  # Train BART model on YB
  bart2b <- dbarts::bart(x.train = xt, y.train = YB, x.test = rbind(xt1, xt0), verbose = FALSE)
  # Extract predicted outcomes
  mu1_hat <- colMeans(bart2b$yhat.test[, 1:n_good])
  mu0_hat <- colMeans(bart2b$yhat.test[, (n_good + 1):(2 * n_good)])
  # AIPW estimate
  aipw_te <- mean(
    mu1_hat - mu0_hat +Z_good * (YB - mu1_hat) / ps_est -
      (1 - Z_good) * (YB - mu0_hat) / (1 - ps_est))
  # Store results
  results.b[i, 14] <- tauBs - aipw_te
  aipw_var <- var(mu1_hat - mu0_hat + Z_good * (YB - mu1_hat) / ps_est - 
                    (1 - Z_good) * (YB - mu0_hat) / (1 - ps_est)) / n_good
  ci <- c(aipw_te - 1.96 * sqrt(aipw_var), aipw_te + 1.96 * sqrt(aipw_var))
  results.b[i, 15] <- (ci[1] < tauBs & ci[2] > tauBs) * 1
  results.b[i, 16] <- ci[2] - ci[1]
  precision.b[i, 5] <- sqrt(mean((aipw_te - tauBis)^2))
  
  # Train BART model on YC
  bart2c <- dbarts::bart(x.train = xt, y.train = YC, x.test = rbind(xt1, xt0), verbose = FALSE)
  # Extract predicted outcomes
  mu1_hat <- colMeans(bart2c$yhat.test[, 1:n_good])
  mu0_hat <- colMeans(bart2c$yhat.test[, (n_good + 1):(2 * n_good)])
  # AIPW estimate
  aipw_te <- mean(
    mu1_hat - mu0_hat +Z_good * (YC - mu1_hat) / ps_est -
      (1 - Z_good) * (YC - mu0_hat) / (1 - ps_est))
  # Store results
  results.c[i, 14] <- tauCs - aipw_te
  aipw_var <- var(mu1_hat - mu0_hat + Z_good * (YC - mu1_hat) / ps_est - 
                    (1 - Z_good) * (YC - mu0_hat) / (1 - ps_est)) / n_good
  ci <- c(aipw_te - 1.96 * sqrt(aipw_var), aipw_te + 1.96 * sqrt(aipw_var))
  results.c[i, 15] <- (ci[1] < tauCs & ci[2] > tauCs) * 1
  results.c[i, 16] <- ci[2] - ci[1]
  precision.c[i, 5] <- sqrt(mean((aipw_te - tauCis)^2))
